...
<div style="display: flex; justify-content: center; align-items: center; margin-top:5%; margin-bottom: 5% ">
    <div style="max-width: 40vw">
        <form method="post" action="{{ url('/Cart', $confirmOrder->id) }}">
            {{ csrf_field() }}
            {{ method_field('PUT') }}

            <div class="res_holder">
                <div class="img_holder">
                    <img src="{{ asset('img/'. $confirmOrder->title. '.jpg') }}" alt="biryani" style="width:100%">
                </div>
                <div  class="rest_info_holder">
                    <div >
                        <nav class="rest_name">
                            <span class="rest_title" >{{$confirmOrder->title}}</span>
                            <input type="hidden" name="title" value="{{$confirmOrder->title}}"/>
                        </nav>
                        <p class="rest_desc" name="description" >{{$confirmOrder->description}}</p>
                        <span class="cost" name="cost" >Price: ${{$confirmOrder->cost}}</span>
                        <input type="text" name="description" value="{{$confirmOrder->description}}"/>
                        <input type="text"  name="cost" value="{{$confirmOrder->cost}}"/>
                    </div>
                    ...
                    <div style="margin-bottom: 10px">
                        ...
                        <label class="rest_desc">Special instructions for the dish</label> <span id="issues" ></span></br>
                        <textarea class="rest_desc" rows="2" cols="32" id="issues" name="instructions" > </textarea></br>

                        <button class="rest_desc" type="submit" name="submit" value="Send Feedback" id="submitId" /> Update cart </button>
                    </div>
